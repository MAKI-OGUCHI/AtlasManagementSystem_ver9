      ...
      foreach($days as $day){
       ...
       if($startDay <= $day->everyDay() && $toDay >= $day->everyDay()){
        ...
        if($day->everyDay()){
          //過去日
          if($startDay <= $day->everyDay() && $toDay >= $day->everyDay()){
            $html[] = '<p class="d-flex m-0 p-0">1部<input class="w-25 past-day" style="height:20px;" name="reserve_day['.$day->everyDay().'][1]" type="text" form="reserveSetting" value="'.$day->onePartFrame($day->everyDay()).'" disabled></p>';
